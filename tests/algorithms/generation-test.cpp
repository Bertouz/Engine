...
SCENARIO("We want to be able to compute linear spaced range between two given values",
         "[algorithms][generation][range]")
{
    GIVEN("a vector with a size n = 50 and dt = 5")
    {
        constexpr int n = 10;
        ...
        WHEN("calling ngn::range(begin, end, -5, 5)")
        {
            ...
            {
                std::vector<double> ref_linspace{
                    -5.0,         -3.8888888889, -2.7777777778, -1.6666666667, -0.5555555556,
                    0.5555555556, 1.6666666667,  2.7777777778,  3.8888888889,  5.0};
        ...
        WHEN("calling ngn::range(begin, end, 0, 10)")
        {
            ...
            {
                std::vector<double> ref_linspace{0.0,          1.1111111111, 2.2222222222, 3.3333333333, 4.4444444444,
                                                 5.5555555556, 6.6666666667, 7.7777777778, 8.8888888889, 10.0};
...
SCENARIO("We want to be able to compute an amplitude modulated waveform",
         "[algorithms][generation][amplitude_modulated_waveform]")
{
    GIVEN("Some parameters")
    {
        ...
        {
            ngn::amplitude_modulated_waveform(t.begin(), t.end(), signal.begin(), amp_carrier, freq_carrier_Hz,
                                              amp_modu, freq_modu_Hz);

            THEN("We have an amplitude modulated waveforme")
            {

                std::vector<double> ref_waveform{0.0,           -0.0914985754,
                                                 -0.2006721044, -0.3570844584,
                                                 -0.6357488894, -1.3449970239,
                                                 -7.7089328815, 2.9076279039,
                                                 1.4022053526,  1.0037421793,
                                                 0.8312538756,  0.7470418822,
                                                 0.7113204416,  0.7113204416,
                                                 0.7470418822,  0.8312538756,
                                                 1.0037421793,  1.4022053526,
                                                 2.9076279039,  -7.7089328815,
                                                 -1.3449970239, -0.6357488894,
                                                 -0.3570844584, -0.2006721044,
                                                 -0.0914985754, 0.0};
