    ...
    GIVEN("an input stream with csv formatted data")
    {
        ...
        WHEN("using parse_csv(*inputs, table);")
        {
            ...
            THEN("We extract correctly the data")
            {
                ...
                std::vector<double> ref_volts{ 
                    -0.02503107, -0.00430085, 0.01638979, 0.02664651, 0.02684325, 0.0269058,
                    0.02681732, 0.02657055, 0.02616828, 0.0256231, 0.02495668, 0.02419838,
                    0.02338336, 0.02255038};
