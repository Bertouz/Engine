...
PicoApplication::PicoApplication(int argc, char** argv) :
  cli_app("pico is a little tool to compute the frequency of the highest amplitude wave in a waveform from file")
{
  this->cli_app.add_option(
    "-f,--file",
    inputs,
    "File containing the data of the waveform we want to analyze, if its a directory then we look for all files matching the format at the first level");
  ...
  if (this->inputs.empty())
  {
    throw std::runtime_error(
      "You didn't provide any input file, for more informations refer to the help using -h or --help");
  }
...
void PicoApplication::exec() const
{
  ...
  if (std::filesystem::is_directory(this->inputs))
  {
    ...
    for (auto ite = std::filesystem::directory_iterator(this->inputs); ite != std::filesystem::directory_iterator();
         ite++)
    {
  ...
  for (const auto& curr_waveform_path : input_waveforms)
  {
    ...
    if (curr_table.size() == 2)
    {
      ...
      std::transform(time.begin(), time.end(), time.begin(), [](auto v) { return v * 1'000'000; });
